...
options.save = function() {
  console.log("options.save = function()");
  localStorage.options = JSON.stringify(options);
};

var timeoutNoResponse,
  timeoutSearch;
...
function resetDatabase(callback) {
  console.log("resetDatabase(callback)");
...
function getArchiveURL(id, editMode, callback) {
  console.log("getArchiveURL(id, editMode, callback)");
  function createObjectURL(content, title) {
    console.log("createObjectURL(content, title)");
    var array = [
...
function open(id, selected, editMode) {
  console.log("open(id, selected, editMode)");
...
function detectSingleFile(callback) {
  console.log("detectSingleFile(callback)");
  var img = new Image();
  ...
  img.onerror = function() {
    console.log("img.onerror = function()");
    callback(false);
  };
}
...
function exportDB() {
  ...
  process.exporting = {
    ...
    cancel: storage.exportDB(function(index, max) {
      console.log("storage.exportDB(function(index, max)");
...
function cancelExportDB() {
  console.log("cancelExportDB()");
  process.exporting.cancel();
  process.exporting = null;
}

function refreshBadge(text, title) {
  console.log("refreshBadge(text, title)");
...
function exportToZip(checkedPages, filename) {
  console.log("exportToZip(checkedPages, filename)");
  var pctIndex = 0;
...
function importFromZip(file) {
  console.log("importFromZip(file)");
  var pctIndex = 0;
  if (process.exportingToZip || process.importingFromZip)
    return;
...
chrome.omnibox.onInputChanged.addListener(function(text, suggestCallback) {
  console.log("chrome.omnibox.onInputChanged.addListener(function(text, suggestCallback)");
  if (text) {
    if (timeoutSearch)
...
chrome.omnibox.onInputEntered.addListener(function(text, suggestCallback) {
  console.log("chrome.omnibox.onInputEntered.addListener(function(text, suggestCallback)");
  var id,
    match = text.match(/\(([^)]*)\)$/);
  if (match)
    id = text.match(/\(([^)]*)\)$/)[1];
...
chrome.extension.onMessage.addListener(function(request, sender, sendResponse) {
  console.log("chrome.extension.onMessage.addListener(function(request, sender, sendResponse)");
...
chrome.extension.onMessageExternal.addListener(function(request, sender, sendResponse) {
  console.log("chrome.extension.onMessageExternal.addListener(function(request, sender, sendResponse)");
  setTimeoutNoResponse();
