...
function ComboBox(element, value) {
  var timeoutDataChange,
    timeoutHide,
    that = this,
    suggest = document.createElement("select"),
    wrapper = document.createElement("span"),
    data = document
      .getElementById(element.getAttribute("list")),
    inputRegExp = / combobox-input/,
    resetElement = element.resetElement;
  ...
  element.__defineGetter__("values", function() {
    console.log('element.__defineGetter__("values", function() {');
...
function ElementInput(element, value) {
  var elementRegExp = / element-input/,
    resetElement = element.resetElement;
  ...
  element.__defineSetter__("value", function(value) {
    console.log('element.__defineSetter__("value", function(value) {');
    element.textContent = value;
  });
  element.__defineGetter__("value", function() {
    console.log('element.__defineGetter__("value", function() {');
...
function RatingInput(element, value) {
  ...
  function setRating(rating) {
    var className = element.className,
      ratingRegExp = / rating-value-\d\d\d/;
    if (rating)
  ...
  element.addEventListener("mousemove", function(event) {
    console.log('element.addEventListener("mousemove", function(event) {');
    setRating(Math.ceil(event.offsetX / starWidth) * 100);
  }, true);
  element.addEventListener("mouseout", function(event) {
    console.log('element.addEventListener("mouseout", function(event) {');
    setRating(value);
  }, true);
  ...
  element.__defineSetter__("value", function(val) {
    console.log('element.__defineSetter__("value", function(val) {');
    setRating(val);
    value = val;
  }, true);
  element.__defineGetter__("value", function() {
...
function KeywordsInput(element, values, tagsLabel, addTagtitle, deleteTagTitle, tagTitle) {
  var timeoutBlur,
    currentKeyword,
    plusElement = document.createElement("img"),
    dataNewTag = document.createElement("datalist"),
    tagsLabelElement = document
      .createElement("span"),
    currentValues = [];
  ...
  function decorateKeywordElement() {
    ...
    currentKeyword.addEventListener("keyup", function(event) {
      console.log('currentKeyword.addEventListener("keyup", function(event) {');
      if (event.keyIdentifier == "Enter")
        keywordOnclick();
    }, false);
    ...
    deleteElement.addEventListener("keyup", function(event) {
      console.log('deleteElement.addEventListener("keyup", function(event) {');
      if (event.keyIdentifier == "Enter")
        deleteOnclick();
    }, false);
  ...
  plusElement.addEventListener("mousedown", function() {
    console.log('plusElement.addEventListener("mousedown", function() {');
    if (event.button == 0) {
      onmousedown();
      event.preventDefault();
    }
  ...
  plusElement.addEventListener("keydown", function(event) {
    console.log('plusElement.addEventListener("keydown", function(event) {');
    if (event.keyIdentifier == "Enter") {
      onmousedown();
      event.preventDefault();
    }
  ...
  element.__defineSetter__("values", function(values) {
    console.log('element.__defineSetter__("values", function(values) {');
    if (currentKeyword)
    ...
    Array.prototype.forEach.call(element.querySelectorAll(".keywords-input-keyword-wrapper"), function(wrapper) {
      console.log('Array.prototype.forEach.call(element.querySelectorAll(".keywords-input-keyword-wrapper"), function(wrapper) {');
      element.removeChild(wrapper);
    });
    values.forEach(function(value) {
      currentKeyword = document.createElement("span");
...
function TitleInput(element, value, editButtonTitle, deleteButtonTitle) {
  ...
  wrapper.parentElement.addEventListener("mouseover", function() {
    console.log('wrapper.parentElement.addEventListener("mouseover", function() {');
    if (!currentTitleEdit)
      commandWrapper.style.opacity = 1;
  }, true);
  wrapper.parentElement.addEventListener("mouseout", function() {
    console.log('wrapper.parentElement.addEventListener("mouseout", function() {');
    commandWrapper.style.opacity = 0;
  }, true);
  imgElement.addEventListener("mousedown", function() {
    console.log('imgElement.addEventListener("mousedown", function() {');
    clearTimeoutBlur();
    if (currentTitleEdit) {
      value = element.textContent;
      resetElement();
  ...
  deleteElement.addEventListener("mousedown", function() {
    console.log('deleteElement.addEventListener("mousedown", function() {');
...
function CollapserButton(element, contentElement, expanded, expandTitle, collapseTitle) {
  ...
  element.addEventListener("keyup", function(event) {
    console.log('element.addEventListener("keyup", function(event) {');
    if (event.keyIdentifier == "Enter")
      toggleContent();
  }, false);
...
function PeriodInput(element, value, fromDateUser, toDateUser, fromDateLabel, toDateLabel, allLabel, emptyLabel, todayLabel, weekLabel, monthLabel, userLabel) {
  var period,
    ...
    fromLabel = document
      .createElement("span"),
    toLabel = document.createElement("span"),
    periodContainer = document.createElement("span"),
    container = document
      .createElement("span"),
    periodInput = document.createElement("select");
  ...
  element.__defineGetter__("fromDate", function() {
    console.log('element.__defineGetter__("fromDate", function() {');
    var dateArray;
  ...
  element.__defineGetter__("toDate", function() {
    console.log('element.__defineGetter__("toDate", function() {');
