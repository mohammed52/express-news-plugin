...
(function() {
  ...
  var bgPage = chrome.extension.getBackgroundPage(),
    ...
    toRatingInput,
    expandMiscButton,
    expandReadDateButton,
    expandSavedDateButton,
    expandUrlButton,
    expandTagsButton,
    savedPeriodInput,
    readPeriodInput,
  ...
  function tagsInputOninput() {
    var datalist = document.getElementById("pages-tags-filter-data"),
    ...
    bgPage.storage.getTagsCompletion(filteredTags, tag, function(tags) {
      console.log('bgPage.storage.getTagsCompletion(filteredTags, tag, function(tags) {');
      var i,
        optionElement;
  ...
  function selectAllButtonOnclick() {
    Array.prototype.forEach.call(document.querySelectorAll("#tab-pages li input[type=checkbox]"), function(inputElement) {
      console.log('Array.prototype.forEach.call(document.querySelectorAll("#tab-pages li input[type=checkbox]"), function(inputElement) {');
      var id = inputElement.parentElement.id.split("page.")[1],
        index;
    ...
    if (checkedPages.length)
      if (bgPage.options.askConfirmation != "yes" || confirm("Do you really want to delete selected archives ?"))
        bgPage.storage.deletePages(checkedPages, function() {
          console.log('bgPage.storage.deletePages(checkedPages, function() {');
          checkedPages = [];
          showPagesTab();
        });
  }
  ...
  function tagButtonOnclick() {
    if (checkedPages.length) {
      ...
      bgPage.storage.getSharedTags(checkedPages, function(tags) {
        console.log('bgPage.storage.getSharedTags(checkedPages, function(tags) {');
  ...
  function tagOkButtonOnclick() {
    bgPage.storage.addTags(tagInput.initValues, tagInput.values, checkedPages, function() {
      console.log('bgPage.storage.addTags(tagInput.initValues, tagInput.values, checkedPages, function() {');
      search(null, true);
    });
      ...
      (function(row, rowTags) {
        ...
        aElement.ondelete = function() {
          if (confirm("Do you really want to delete this archive ?"))
            bgPage.storage.deletePages([row.id], function() {
              console.log('bgPage.storage.deletePages([ row.id ], function() {');
              showPagesTab();
            });
        };
        ...
        infoLine3Element.oninput = function(event) {
          var input = event.target,
            data = document.getElementById(event.target.list);
          data.innerHTML = "";
          bgPage.storage.getTagCompletion(input.textContent, row.id, function(tags) {
            console.log('bgPage.storage.getTagCompletion(input.textContent, row.id, function(tags) {');
            var i,
              optionElement,
              select = document.createElement("select");
            for (i = 0; i < tags.length; i++) {
  ...
  function search(callback, dontResetCurrentPage) {
    ...
    bgPage.storage.search(state.searchFilters, bgPage.options.searchInTitle, function(rows, tags, count) {
      console.log('bgPage.storage.search(state.searchFilters, bgPage.options.searchInTitle, function(rows, tags, count) {');
      var i,
        link,
        start;
      pageCount = count;
  ...
  this.initPagesTab = function() {
    ...
    tagInput.oninput = function(event) {
      ...
      bgPage.storage.getFilteredTagCompletion(tagInput.values, input.textContent, function(tags) {
        console.log('bgPage.storage.getFilteredTagCompletion(tagInput.values, input.textContent, function(tags) {');
        var i,
          optionElement,
          select = document.createElement("select");
        for (i = 0; i < tags.length; i++) {
