...
(function() {
  ...
  storage.importDB = function(onprogress, onfinish) {
    console.log("storage.importDB = function(onprogress, onfinish)");
    var cancelImport = false;

    function importContent(rows, index) {
      console.log("importContent(rows, index)");
      var id,
        content;

      function importNextContent() {
        console.log("importNextContent()");
  ...
  storage.importFromZip = function(file, onprogress, onfinish) {
    console.log("storage.importFromZip = function(file, onprogress, onfinish)");
    var importIndex = 0,
      zipReader;
  ...
  storage.exportDB = function(onprogress, onfinish) {
    console.log("storage.exportDB = function(onprogress, onfinish)");
    var cancelExport = false;

    function exportContent(rows, index) {
      console.log("exportContent(rows, index)");
      var id,
        content;

      function exportNextContent() {
        console.log("exportNextContent()");
  ...
  storage.getContent = function(id, callback, forceUseDatabase, dontSetReadDate) {
    console.log("storage.getContent = function(id, callback, forceUseDatabase, dontSetReadDate)");
    db.transaction(function(tx) {
      var date = new Date();
        ...
        if (fs && !forceUseDatabase)
          fs.root.getFile(id + ".html", null, function(fileEntry) {
            ...
            fileReader.onerror = function(e) {
              console.log("fileReader.onerror = function(e)");
              if (callback)
  ...
  storage.addTag = function(pageId, tag, callback) {
    console.log("storage.addTag = function(pageId, tag, callback)");
    var tagId;
    db.transaction(function(tx) {
  ...
  storage.addTags = function(initTags, tags, pageIds, callback) {
    console.log("storage.addTags = function(initTags, tags, pageIds, callback)");
    var tagIndex = 0,
      pageIndex = 0,
      toAdd = diff(tags, initTags),
      toRemove = diff(initTags, tags);
  ...
  storage.deleteTags = function(tagIds, callback) {
    console.log("storage.deleteTags = function(tagIds, callback)");
    db.transaction(function(tx) {
      var i,
        query,
        params = [];
  ...
  storage.getTags = function(tags, callback) {
    console.log("storage.getTags = function(tags, callback)");
    var requestUsed,
      requestUnused;
  ...
  storage.getTagsCompletion = function(filteredTags, tag, callback) {
    console.log("storage.getTagsCompletion = function(filteredTags, tag, callback)");
    db.transaction(function(tx) {
      var i,
        query,
        params = [],
  ...
  storage.getSharedTags = function(pageIds, callback) {
    console.log("storage.getSharedTags = function(pageIds, callback)");
    db.transaction(function(tx) {
      var i,
        query,
        params = [];
  ...
  storage.getFilteredTagCompletion = function(filteredTags, tag, callback) {
    console.log("storage.getFilteredTagCompletion = function(filteredTags, tag, callback)");
    db.transaction(function(tx) {
      var i,
        query = "select tag from tags where 1=1",
        params = [];
  ...
  storage.getTagCompletion = function(searchTag, pageId, callback) {
    console.log("storage.getTagCompletion = function(searchTag, pageId, callback)");
    db.transaction(function(tx) {
      var query = "select tag from tags where 1=1",
        params = [];
      if (searchTag)
  ...
  function getDates(period, fromDateUser, toDateUser) {
    console.log("getDates(period, fromDateUser, toDateUser)");
    var dates = {},
      fromDate,
      toDate;
    switch (period) {
  ...
  function buildSearchPagesQuery(searchFilters, searchInTitle, params, tags, tagIndex) {
    console.log("buildSearchPagesQuery(searchFilters, searchInTitle, params, tags, tagIndex)");
    var i,
      dates,
  ...
  function buildFullSearchPagesQuery(searchFilters, searchInTitle, params, count) {
    console.log("buildFullSearchPagesQuery(searchFilters, searchInTitle, params, count)");
    var query,
      tagIndex,
      queries = [],
      excludeTags,
  ...
  storage.search = function(searchFilters, searchInTitle, callback) {
    console.log("storage.search = function(searchFilters, searchInTitle, callback)");
    db.transaction(function(tx) {
      var params = [],
        query = buildFullSearchPagesQuery(searchFilters, searchInTitle, params);
      query += " order by";
  ...
  storage.setTitle = function(id, title) {
    console.log("storage.setTitle = function(id, title)");
    db.transaction(function(tx) {
  ...
  storage.deletePages = function(pageIds, callback) {
    console.log("storage.deletePages = function(pageIds, callback)");
    db.transaction(function(tx) {
      var i,
        query,
        params = [];
  ...
  function removeNullChar(content) {
    console.log("removeNullChar(content)");
  ...
  function addContent(favicoData, url, title, content, text, timestamp, readDateTs, idx, tags, callback, onFileErrorCallback, forceUseDatabase) {
    console.log("addContent(favicoData, url, title, content, text, timestamp, readDateTs, idx, tags, callback, onFileErrorCallback, forceUseDatabase)");
  ...
  storage.addContent = function(content, title, url, favicoData, callback, onFileErrorCallback) {
    console.log("storage.addContent = function(content, title, url, favicoData, callback, onFileErrorCallback)");
    var titleLength,
      domain,
      domainArray,
      node,
      ...
      pageInfoArray,
      tagsArray,
      newDoc = document.implementation
        .createHTMLDocument();
    newDoc.open();
    newDoc.writeln(content);
    newDoc.close();
  ...
  storage.setRating = function(id, rating) {
    console.log("storage.setRating = function(id, rating)");
    if (rating)
      db.transaction(function(tx) {
  ...
  storage.reset = function(callback) {
    console.log("storage.reset = function(callback)");
    db.transaction(function(tx) {
